...
import CreateAccountModal from '../modals/signup'; // New import
...
export default function DashboardLayout() {
 ...
 const handlers = useSwipeable({
  ...
  const handleCreate = data => setCreateOpen(false);
  ...
  return (
    <Flex h="100vh" bg={theme==='light'?'#f9f9f9':'#111'} color={theme==='light'?'black':'white'} fontSize="sm" overflow="hidden">
        ...
        <VStack w="220px" bg={theme==='light'?'#fff':'#000'} p={2} spacing={2} minH="100vh">
          <HStack gap={0}>
            ...
            <VStack align="start" gap={0}>
              <Text fontSize="xs" fontWeight="bold"><Text as="span" color="green.400">Agro</Text><Text as="span" color="blue.400">Strings</Text></Text>
              <Text fontSize="5px" color="gray.300">XYLOPHONE TECHNOLOGIES</Text>
            </VStack>
          </HStack>
          {sidebarItems.map(({ label, icon }, i) => (
            <HStack key={i} w="full" px={3} py={2} onClick={() => setActiveIndex(i)}
              bg={activeIndex===i?'gray.700':'transparent'} _hover={{bg:'gray.800',cursor:'pointer'}} borderRadius="md"
            >
      ...
      <Flex {...handlers} flex="1" flexDir="column" overflow="hidden">
        {/* Mobile Header */}
        <Flex display={{ base: 'flex', md: 'none' }} align="center" justify="space-between" px={4} py={3}
          bg={theme==='light'?'#fff':'#111'} borderBottom="1px solid" borderColor="gray.700" zIndex="10"
        >
          <HStack>
            <Image src={logo} alt="Logo" boxSize="40px" />
            <VStack align="start" gap={0}>
          ...
          <HStack spacing={2}>
            <Button size="xs" color="#fada25" bg="transparent" border="1px solid" onClick={() => {
              setSignInOpen(true);
              setCreateOpen(false);
            }}>
              Sign In
...
/>
      {/* Mobile Bottom Nav */}
      <Box display={{ base: 'block', md: 'none' }} pos="fixed" bottom="0" left="0" right="0" h="70px"
        bg="#000" borderTop="1px solid #333" zIndex="99"
      >
        <Flex h="100%" align="center">
          {mobileTabs.map(({ label, icon, index }) => (
            <Flex key={index} flex="1" direction="column" align="center" justify="center" cursor="pointer"
        ...
        <Box pos="absolute" left="50%" top="8" transform="translate(-50%,-50%)">
          <Circle size="54px" bg="#fada25" border="4px solid #000" cursor="pointer"
