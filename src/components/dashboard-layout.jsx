...
import { Box, Flex, VStack, HStack, Input, Icon, Button, Image, Circle, Text, Spacer, Link } from '@chakra-ui/react';
import { Home, Video, Upload, Settings, TvIcon, Search, Menu, CirclePlus } from 'lucide-react';
...
const DashboardLayout = () => {
  ...
  const sidebarItems = [
    ...
    { label: 'Products', icon: Settings /* placeholder use Search or other icon */ },
    ...
    <Flex h="100vh" bg="#111" color="white" fontSize="sm" overflow="hidden">
      ...
      <Box display={{ base: 'none', md: 'block' }}>
        <VStack w="220px" bg="#000" p={2} spacing={2} minH="100vh" align="start">
          ...
          <Text fontSize="xs" color="gray.600" mt="auto">@XylophoneTechnologies Uganda</Text>
        </VStack>
      </Box>
      ...
      <Flex flex="1" flexDir="column">
        ...
        <Flex display={{ base: 'none', md: 'flex' }} mb={4} px={4} pt={4} align="center" justify="space-between">
          ...
          <HStack spacing={2} fontSize="xs">
            <Box border="2px dashed gray.500" borderRadius="sm" p={1}>
              <CirclePlus size={18} />
            </Box>
        ...
        {activeIndex === 1 && (
          <Box flex="1" p={4}><Text>Live channel content coming soon.</Text></Box>
        )}
        {activeIndex === 2 && (
          <Box flex="1" p={4}><Text><FarmerChannel /></Text></Box>
        )}
        {activeIndex === 3 && (
          <Box flex="1" p={4}><Text>Upload Video UI goes here.</Text></Box>
        )}
        {activeIndex === 4 && (
          <Box flex="1" p={4} overflowY="auto" bg="#222"><ProductListing /></Box>
        )}
        {activeIndex === 5 && (
          <Box flex="1" p={4}><Text fontSize="lg" color="gray.300">Settings Page (Coming soon)</Text></Box>
        )}
      </Flex>
